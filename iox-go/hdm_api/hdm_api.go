...
func Show_cmd_ssh_ap(ip string, command string) (string){
	 ...
	 if ip != "unassigned" {
		 hostname = cfg.Section("ap_info").Key("IP").String()
	 }else {
		 hostname = cfg.Section("ap_info").Key("IP").String()
	 }
         port := cfg.Section("ap_info").Key("port").String()

         username := cfg.Section("ap_info").Key("user").String()
	 password := cfg.Section("ap_info").Key("pass").String()
         config := &ssh.ClientConfig{
                 User: username,
                 Auth: []ssh.AuthMethod{ssh.Password(password)},
                 HostKeyCallback: func(hostname string, remote net.Addr, key ssh.PublicKey) error {
